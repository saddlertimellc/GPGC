...
We auto-discover GPIO lines by *name* across /dev/gpiochip* so you don't need offsets.
...
USE_HW_CS = True               # leave kernel CS enabled; we also toggle our GPIO CS

LCD_WIDTH, LCD_HEIGHT = 320, 240

# GPIO line *names* (not offsets)
LCD_CS_NAME  = "GPIO1_C0_d"
LCD_DC_NAME  = "GPIO2_A7_d"
LCD_RST_NAME = "GPIO1_D3_d"
LCD_BL_NAME  = "GPIO2_A6_d"
TP_CS_NAME   = "GPIO1_B2_d"
TP_IRQ_NAME  = "GPIO1_B3_u"
...
# ----- libgpiod v2 helpers (discover lines by name) -----
def _all_chips():
...
def _find_line(chip, offset):
    """Return gpiod.LineInfo for offset (v2)"""
    return chip.get_line_info(offset)

def _find_by_name(line_name: str):
    """Search all gpiochips for a line with the given name. Return (chip_path, offset)."""
    for chip_path in _all_chips():
        chip = gpiod.Chip(chip_path)
        try:
            # iterate all lines on this chip
            for off in range(chip.num_lines):
                info = _find_line(chip, off)
                if info and info.name == line_name:
                    return chip_path, off
        finally:
            chip.close()
    raise ValueError(f'GPIO line named "{line_name}" not found on any gpiochip')

def _req_out_by_name(line_name: str, consumer: str, default=0):
    chip_path, off = _find_by_name(line_name)
    cfg = {off: gpiod.LineSettings(direction=gpiod.line.Direction.OUTPUT,
                                   output_value=int(bool(default)))}
    req = gpiod.request_lines(chip_path, consumer=consumer, config=cfg)
    return (chip_path, off, req)

def _req_in_by_name(line_name: str, consumer: str):
    chip_path, off = _find_by_name(line_name)
    cfg = {off: gpiod.LineSettings(direction=gpiod.line.Direction.INPUT)}
    req = gpiod.request_lines(chip_path, consumer=consumer, config=cfg)
    return (chip_path, off, req)

def _set(req_tuple, val):
    chip_path, off, req = req_tuple
    req.set_values({off: int(bool(val))})

def _get(req_tuple):
    chip_path, off, req = req_tuple
    return req.get_values([off])[off]
# --------------------------------------------------------


class GPIOOut:
    def __init__(self, name, consumer, default=0):
        self.name = name
        self.t = _req_out_by_name(name, consumer, default)
    def set(self, v): _set(self.t, v)
...
class GPIOIn:
    def __init__(self, name, consumer):
        self.name = name
        self.t = _req_in_by_name(name, consumer)
    def get(self): return _get(self.t)
    def __enter__(self): return self
    def __exit__(self, *a):
        self.t[2].release()
...
def open_spidev(node, speed, mode=0):
    ...
    spi.bits_per_word = 8
    spi.cshigh = False
    spi.no_cs = False        # your kernel rejected no_cs
    return spi
...
def touch_loop():
    ...
    with GPIOOut(TP_CS_NAME, "tp-cs", 1) as cs, GPIOIn(TP_IRQ_NAME, "tp-irq") as irq:
...
def main():
    ...
    with GPIOOut(LCD_DC_NAME,  "lcd-dc") as dc, \
         GPIOOut(LCD_RST_NAME, "lcd-rst") as rst, \
         GPIOOut(LCD_BL_NAME,  "lcd-bl") as bl, \
         GPIOOut(LCD_CS_NAME,  "lcd-cs", 1) as cs:
